...
- 2026-09-14: Migración del backend de Node/Express a Python/FastAPI en curso (Caso 4, sin cambio de comportamiento) — Fase 1 (crear/listar/eliminar) completada en `backend_py/`, corriendo en paralelo al backend Node en el puerto `4001` (mismo `backend/data/notas.db`, mismos datos). El backend en uso real para el frontend sigue siendo `backend/` (Node, puerto `4000`) hasta cerrar la Fase 2 (recordatorios) y la Fase 3 (corte) del plan. *Porqué: decisión explícita del usuario; ver `docs/plan-migracion-fastapi-2026-09-14.md` para el detalle de fases.*
- 2026-09-14: Migración del backend a Python/FastAPI, Fase 2 (recordatorios) completada — `backend_py/` valida `fechaLimite` (formato + no pasada) y calcula `vencida` al listar, con paridad completa confirmada contra `spec-gestion-notas.md` (26/26 tests de caracterización en verde, iguales contra `backend` Node y contra `backend_py`). `backend_py` ya tiene el 100% del comportamiento de esta capacidad, pero **todavía no es el backend en uso real** — el frontend sigue apuntando a `backend/` (Node, puerto `4000`) hasta la Fase 3 (corte). *Porqué: decisión explícita del usuario; ver `docs/plan-migracion-fastapi-2026-09-14.md`.*
