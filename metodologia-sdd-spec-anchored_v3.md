...
### Cómo decidir el tamaño de una capacidad

No hay una regla de tamaño fijo ("una capacidad = un endpoint" o "= un módulo"), pero sí un criterio para decidir dónde cortar: **si dos comportamientos casi siempre cambian juntos y comparten los mismos invariantes, van en el mismo spec; si pueden evolucionar por separado, van en specs separados.** Por ejemplo, crear un pedido y consultar su historial casi siempre se tocan en el mismo cambio y comparten el mismo modelo de datos — normalmente conviene un único spec. En cambio, autenticación e inventario casi nunca cambian juntos — van separados aunque ambos toquen "pedidos".

Lo que importa más que el tamaño exacto es la **consistencia dentro de un mismo sistema**: si ya existe un spec único que cubre todo un dominio (p. ej. `spec-pedidos.md` con crear + consultar + cambiar estado), no empezar a partir ese mismo dominio en specs sueltos por operación a mitad de camino — eso vuelve ambigua la regla de "cada comportamiento vive en exactamente un spec" (¿en cuál de los dos vive?) y rompe la trazabilidad para quien compare specs entre capacidades. Si hace falta cambiar el criterio de corte, es en sí mismo un cambio deliberado (se explicita en el plan, no se decide implícitamente al redactar un spec nuevo).
